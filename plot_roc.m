function auc = plot_roc(y,x,params,c,g)
...
% 	plotroc(y,x,model);
	...
	elseif exist('params') && ~ischar(params)
		...
		if fold <= 1
			...
			[deci,label_y] = get_cv_deci(y,x,param,fold,c,g); % get the value of decision and label after cross-calidation
...
function [deci,label_y] = get_cv_deci(prob_y,prob_x,param,nr_fold,c,g)
	l=length(prob_y);
	deci = ones(l,1);
	label_y = ones(l,1);	 
	rand_ind = randperm(l); 
	for i=1:nr_fold % Cross training : folding
		...
		train_ind = [1:l]';
		train_ind(test_ind) = [];
		model = svmtrain(prob_y(train_ind),prob_x(train_ind,:),[' -c ',num2str( c ),' -g ',num2str( g ),' -b 1']);	   
